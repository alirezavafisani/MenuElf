...
export default function CreatePlanScreen() {
  ...
  const handleCreate = async () => {
    if (!planName.trim()) { Alert.alert('Name required', 'Please enter a name for the plan.'); return; }
    if (selectedIds.size === 0) { Alert.alert('Select friends', 'Select at least one friend to invite.'); return; }

    setCreating(true);
    try {
      const res = await apiPost('/plans', { name: planName.trim(), friend_ids: Array.from(selectedIds) });
      if (res.ok) {
        const data = await res.json();
        const planId = data.plan?.id;
        if (planId) router.replace(`/group/${planId}`); else router.back();
      } else {
    ...
    <SafeAreaView style={styles.container} edges={['top', 'bottom']}>
      ...
      <View style={styles.content}>
        ...
        {loading ? (
          ...
          <View style={styles.center}><Text style={styles.emptyText}>No friends yet. Add friends first!</Text></View>
        ) : (
          <FlatList
            data={friends}
      ...
      <View style={styles.footer}>
        ...
        <GoldButton title="Create Plan" onPress={handleCreate} loading={creating} disabled={selectedIds.size === 0} />
      </View>
    </SafeAreaView>
  );
}
...
const styles = StyleSheet.create({
  ...
  friendRowSelected: { borderWidth: 1, borderColor: colors.accent, backgroundColor: colors.accentLight },
  ...
  footer: { padding: spacing.screenPadding, borderTopWidth: 1, borderTopColor: colors.border },
  selectedCount: { fontSize: 13, color: colors.textSecondary, textAlign: 'center', marginBottom: 10 },
});
