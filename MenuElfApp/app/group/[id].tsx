...
export default function GroupChatScreen() {
  ...
  useEffect(() => { if (planId) loadPlan(); }, [planId]);
  ...
  const loadPlan = async () => {
    setLoadingPlan(true);
    try { const res = await apiGet(`/plans/${planId}`); if (res.ok) { const data = await res.json(); setPlan(data.plan); } }
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (polling && lastTimestampRef.current) {
          ...
          setMessages(newMsgs);
          if (newMsgs.length > 0) lastTimestampRef.current = newMsgs[newMsgs.length - 1].created_at;
  ...
  const sendMessage = async () => {
    ...
    const optimisticMsg: Message = {
      id: `temp-${Date.now()}`, plan_id: planId || '',
      sender_id: currentUserId, sender_type: 'user',
      content: text, created_at: new Date().toISOString(), sender_profile: null,
    };
    ...
    try {
      ...
      if (res.ok) {
        ...
        setMessages(prev => {
          ...
          if (merged.length > 0) lastTimestampRef.current = merged[merged.length - 1].created_at;
          return merged;
        });
        setTimeout(() => flatListRef.current?.scrollToEnd({ animated: true }), 100);
      } else {
    ...
    return (
      <View style={[styles.msgRow, isMe ? styles.msgRowRight : styles.msgRowLeft]}>
        ...
        <View style={[
          ...
          <Text style={[styles.msgText, isMe && styles.msgTextUser]}>{item.content}</Text>
        </View>
      </View>
    );
  };
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top', 'bottom']}>
      ...
      {isCancelled && (
        <View style={styles.banner}><Text style={styles.bannerText}>This plan was cancelled</Text></View>
      )}
      {isDecided && (
      ...
      >
        <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.center}><Text style={styles.emptyText}>No messages yet. Start the conversation!</Text></View>
          }
        />
...
const styles = StyleSheet.create({
  ...
  bannerSuccess: { backgroundColor: colors.accentLight },
  bannerText: { fontSize: 14, fontWeight: '600', color: colors.accent, textAlign: 'center' },

  messagesList: { padding: spacing.screenPadding, paddingBottom: 8, gap: 10 },
  msgRow: { maxWidth: '85%' },
  msgRowLeft: { alignSelf: 'flex-start' },
  msgRowRight: { alignSelf: 'flex-end' },
  senderLabel: { fontSize: 11, fontWeight: '600', color: colors.textTertiary, marginBottom: 3, marginLeft: 4 },
  ...
  textInput: {
    ...
    fontSize: 15, color: colors.textPrimary, marginRight: 10,
  },
  sendBtn: {
    width: 44, height: 44, borderRadius: 22,
    backgroundColor: colors.accent, justifyContent: 'center', alignItems: 'center',
  },
  sendBtnDisabled: { backgroundColor: colors.backgroundTertiary },
});
